...
- **`src/hooks/useUnits.js`** — all data fetching/merging/persistence, now against the two-table schema.
  - `loadUnits()` — fetches the GeoJSON, plus **both** `units` and `projects` from Supabase (in parallel via `Promise.all`), groups `projects` by `unit_id`, and attaches each unit's matching projects as a `projects: []` array. Runs once on mount.
  - `updateUnit(unit, updates)` — writes **unit-level fields only** (`status`/`occupant`/`phone`/`email`/`universal_key`) to `units`; inserts if `unit.id` is null, otherwise updates.
  - `ensureUnitRow(unit)` — internal helper; bare-inserts a `units` row if one doesn't exist yet, so a project always has a real `unit_id` to point at. Used by `addProject`.
  - `addProject(unit, projectData)` / `updateProject(unit, project, updates)` — insert a new task or update an existing one in `projects`; both return the whole updated unit (with its `projects` array patched) so the caller can re-render with fresh data, same pattern as `updateUnit`.
- **`src/components/MapView.jsx`** — all Mapbox setup and rendering.
  - `addMapLayers(map, unitsRef, hoveredIdRef, openPopup)` — adds the geojson source and the four layers (priority ring, base circles, hover circles, address labels), plus hover/click handlers. Called on initial map load and again after every style swap.
  - `ringPriorityFor(unit)` — collapses a unit's **open** tasks down to one severity for the map ring: `'urgent'` if any open task is urgent, else `'medium'` if any is medium, else `'low'` (no ring). Computed fresh on every `unitsToGeoJSON` call, never stored.
  - `unitsToGeoJSON(units)` — converts unit objects (dot color = `unit.status` directly, ring = `ring_priority` from the helper above) into a GeoJSON `FeatureCollection`, filtering out any unit missing lat/lon.
  - `openPopup(map, unit, lngLat)` / `closePopup()` — mount/unmount the React-rendered `UnitPopup`; `openPopup`'s `renderPopup` closure lets the popup re-render itself with fresh data after any save (unit-level, add-task, or update-task) without closing and reopening.
  - Effects: init map (once), sync source data on `units` change, apply/clear `activeFilter` + the combined ring filter as Mapbox filters, fly-to + auto-open popup on `searchTarget`, swap style on `mapStyle` change.
- **`src/components/UnitPopup.jsx`** — the per-unit edit form, now split across two concerns that save independently because they write to different tables:
  - **Unit-level** (header status `<select>`, Universal Key, Occupant/Phone/Email in the `'details'` view) — local state seeded from `unit`/reset on `unit.id` change, saved together via the bottom **Save Changes** button → `onSave` → `updateUnit`.
  - **Tasks** (the `'status'` view) — `unit.projects` rendered as a list of `TaskRow` components, one collapsible punch-list row each. Each `TaskRow` owns its own `priority`/`notes` edit state and **saves itself** via its own "Save Task" button → `onUpdateProject`, independent of the card's main Save button. The "Mark done" checkbox inside a task saves immediately on click (no separate button) — flips `status` between `'open'`/`'done'` and stamps/clears `date_completed`, struck through via `line-through` when done. A "+ Add Task" control (task type / priority / date / notes) calls `onAddProject`.
  - `TASK_OPTIONS`/`PRIORITY_OPTIONS`/`STATUS_OPTIONS` are the three dropdowns' fixed lists; `STATUS_BUTTON` colors the header pill; `PRIORITY_DOT` colors each task row's dot (mirrors the map ring: gray/white/yellow).
...
- ~~The application code has not been updated for the new `units`/`projects` schema~~ — **done**: `useUnits.js`, `UnitPopup.jsx`, and `MapView.jsx` were rewritten against the current schema (`units.status` manual/independent, `projects.status` binary open/done, `projects.priority` 3 levels) — see the File / function reference above and the build history entry below. Verified end-to-end in-browser: add task → map ring appears; mark done → strikethrough + ring clears; unit status change → dot color + legend count both update independently of task state.
- **Cross-unit filtering/reporting by task type is still not built** — e.g. "show all units needing beams" or "count bathrooms completed Feb–Mar 2026." The `projects` table (indexed on `task` and `status`) supports these queries, but no UI exists for them yet.
- **`TaskRow`'s "Save Task" button only covers priority/notes** — the "Mark done" checkbox saves itself immediately on click, separately. If a user edits priority/notes *and* toggles done without clicking "Save Task" first, the priority/notes edit is lost (only the done-toggle persists). Minor UX rough edge, not yet addressed.
- **No way to un-add a task (delete) from the UI** — matches the deliberate append-only/no-delete history design, but there's currently no recovery path in the UI if a task is added by mistake (would need a direct SQL delete today).
...
19. `category` → `job` → `task` column renames (two passes); `units.status` (manual, 4-value, drives dot color) added back after being dropped in #18, once it was clarified that unit color stays independent of task status; `projects.status` narrowed from 3 values to binary `open`/`done`; `projects.priority` narrowed from 4 values to 3 (`low`/`medium`/`urgent`, dropping `emergency` — noted as a possible future override flag, not built). All via `ALTER TABLE`/`ADD CONSTRAINT` run manually against the live table; `supabase_schema.sql` kept in sync each time.
20. **App code rewritten against the current `units`/`projects` schema** — `useUnits.js` now fetches both tables and groups projects onto each unit; `updateUnit` writes only unit-level fields, with new `addProject`/`updateProject` writing to `projects`. `MapView.jsx`'s ring layer now computes `ring_priority` live per unit (worst open-task priority) instead of reading a stored field. `UnitPopup.jsx` rebuilt around a `TaskRow` list — punch-list style, collapsible, each task saves independently (own "Save Task" button for priority/notes, immediate save on the "Mark done" checkbox, struck through when done) — separate from the card's main Save button, which now only covers unit-level fields. Verified working end-to-end in-browser (see Known gaps).

No specific feature is queued next — check with Cole for current priorities. Known gaps lists what's still open: cross-unit task-type filtering/reporting (not yet built), a minor save-ordering rough edge on `TaskRow`, no task-delete UI, and the older `full_address` duplicate-row cleanup.
